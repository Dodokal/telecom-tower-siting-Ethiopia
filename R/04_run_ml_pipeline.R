# ============================================================================
# Ethiopia Telecom Tower Siting - Full ML Pipeline (v4 final, fast prediction)
# ----------------------------------------------------------------------------
# All fixes baked in:
#   1. Honest nested out-of-fold stacking for spatial CV metrics
...
#   4. terra clamp + pmax workarounds
#   5. PDP wrapper for tidymodels workflows
#   6. Fast national prediction at 1 km (LightGBM only, ~20 min)
#   7. Country mask before prediction so we never predict off-Ethiopia
#   8. 8 publication-quality 300 dpi PNG plots
#
# Inputs:
...
#
# Outputs:
#   K:/ETH TOWERS/results/                    CSVs + run summary
#   K:/ETH TOWERS/results/figures/            8 PNG plots
#
# Total runtime: ~2-3 hours. Slowest step is nested OOF stacking (~60-90 min).
# ============================================================================
...
PRED_AGGREGATION_FACT <- 10   # 100m -> 1km grid for fast national prediction
...
# ---------------------------------------------------------------------------
# 6. NESTED OUT-OF-FOLD STACKING
# ---------------------------------------------------------------------------
cat("\n=== 5. Nested OOF stacking (~60-90 min) ===\n")
...
# ---------------------------------------------------------------------------
# 9. Final fits on full training set
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
cat("\n=== 12. National prediction stack at 1km ===\n")
...
# ---------------------------------------------------------------------------
# 14. Fast national suitability prediction (LightGBM, 1km, ~15-25 min)
# ---------------------------------------------------------------------------
cat("\n=== 13. National suitability surface (LightGBM, 1km) ===\n")
...
demand <- resample(demand, suit_continuous_1km, method = "bilinear")
demand <- rescale01(demand)

dist_tower <- rast(file.path(GEE_DIR, "ETH_dist_to_existing_tower.tif"))
dist_tower <- resample(dist_tower, suit_continuous_1km, method = "bilinear")
gap <- rescale01(dist_tower)

priority_score <- suit_continuous_1km * demand * gap
...
densif <- (suit_continuous_1km >= quantile(values(suit_continuous_1km, na.rm = TRUE),
                                           0.75, na.rm = TRUE)) &
...
cat("Ethiopia Telecom Tower Siting - ML pipeline run summary (v4 final)\n")
